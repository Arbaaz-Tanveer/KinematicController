...
class ROS2RobotSimulation(Node):
    def __init__(self):
        ...
        self.state_pubs = []
        self.odom_pubs = []
        self.obstacle_pubs = []  # New obstacle publishers
        ...
        for i in range(self.num_robots):
            ...
            # Obstacle publisher - publishes all other robots as obstacles
            self.obstacle_pubs.append(
                self.create_publisher(
                    Float32MultiArray,
                    f'{namespace}_obstacles',
                    10
                )
            )
    ...
    def publish_obstacles(self, robot_index: int):
        """Publish obstacle data for a specific robot - all other robots as obstacles"""
        obstacles_msg = Float32MultiArray()
        
        # Calculate obstacles (all other robots except the current one)
        obstacle_data = []
        obstacle_count = 0
        
        for i, robot in enumerate(self.robots):
            if i != robot_index:  # Exclude the current robot
                # Convert to meters and use the same coordinate system as robot state
                x_pos = (robot.x - FIELD_WIDTH/2) / 100.0  # Convert to meters
                y_pos = (FIELD_HEIGHT/2 - robot.y) / 100.0  # Convert to meters
                
                obstacle_data.extend([x_pos, y_pos])
                obstacle_count += 1
        
        # Prepare the message data: [count, x1, y1, x2, y2, ...]
        msg_data = [float(obstacle_count)] + obstacle_data
        
        # Define dimensions
        obstacles_msg.layout.dim = [MultiArrayDimension()]
        obstacles_msg.layout.dim[0].label = "obstacles"
        obstacles_msg.layout.dim[0].size = len(msg_data)
        obstacles_msg.layout.dim[0].stride = len(msg_data)
    ...
    def update_and_render(self):
        ...
        for i, robot in enumerate(self.robots):
            ...
            self.publish_obstacles(i)  # Publish obstacles for each robot
